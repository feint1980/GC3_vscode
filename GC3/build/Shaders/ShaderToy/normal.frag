...
void main() {
		...
		for(i = 0 ; i < rayCount ; i++)
     	{
			...
			{		
				...
				//vec2 aux =  (rayLightPos[i] - fragmentPosition) * 0.01f; //* cos(rayRotation[i] + 1.57079632679) + (rayLightPos[i].y - fragmentPosition.y) * sin(rayRotation[i] + 1.57079632679); 	
     			// aux += 1000.0f * sin(rayRotation[i] + 1.57079632679);
		...
		//vec3 lightAttenuation = vec3(0.025,0.025,0.025);
     	...
     	// float maxColorVal = 4.2f - dayLight.x;
	     // 	if(finalColor.r > maxColorVal)
	     // 	{
	     // 		finalColor.r = maxColorVal;
	     // 	}
	     // 	if(finalColor.g > maxColorVal)
	     // 	{
	     // 		finalColor.g = maxColorVal;
	     // 	}
	     // 	if(finalColor.b > maxColorVal)
	     // 	{
	     // 		finalColor.b = maxColorVal;
	     // 	}
