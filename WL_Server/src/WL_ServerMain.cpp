#include "WL_ServerMain.h"

#include "ServerScriptingManager.h"

WL_ServerMain::WL_ServerMain()
{
    m_password = "TTKR";
    m_port = 1123 ;
}
...
void WL_ServerMain::init(const std::string & password, int port,unsigned int serverSize)
{
    std::cout << "|=========================================|\n";
...
void WL_ServerMain::update(float deltaTime)
{
    // check for packets
...
PacketCode WL_ServerMain::getCommand(const std::string & command)
{
...
void WL_ServerMain::handleInput()
{
    if(kbhit())
    {
...
void WL_ServerMain::handleCommand(const std::string & command)
{
