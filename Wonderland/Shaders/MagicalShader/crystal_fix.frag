...
#else
    #define RAYS_COUNT 27
    #define STEP_MODIFIER 1.25
    #define SHARPNESS 0.275
	#define NOISE_LAYERS_COUNT 4.0
	#define JITTERING 0.005
#endif

#define DITHER 0.27
#define NEAR_PLANE 0.56
#define RENDER_DISTANCE 1.0
...
void main(void)
{
    ...
    // if(length(gl_FragColor.rgb) > 1.0f)
    // {
    //      gl_FragColor.rgb =  gl_FragColor.rgb * 0.3f;//((gl_FragColor.rgb * (1.0 / length(gl_FragColor.rgb))) * 0.8);
    // }
   gl_FragColor.rgb =  gl_FragColor.rgb * 0.3f;
    gl_FragColor.a = 0.8f;
