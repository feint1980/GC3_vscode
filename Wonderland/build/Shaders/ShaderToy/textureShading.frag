...
//uniform float radius[MAX_LIGHT_COUNT];
...
void main() {
    ...
    //you simply do (cos(x) + 1.0) * 0.5
		...
		float radius = 255 /lightAttenuation[i].z;
			...
			if(distance(fragmentPosition,lightPos[i]) < radius)
	     	{
	     		...
	     		float attenuation= 1/ (lightAttenuation[i].x + lightAttenuation[i].y * lapp + lightAttenuation[i].z * lapp * lapp);	
	     		finalColor+= vec4(attenuation,attenuation,attenuation,1.0)*vec4(lightColor[i].rgb,1.0);	
	     	}
     	}
