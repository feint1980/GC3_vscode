...
void ChargingHandler::setCharging(const std::vector<Feintgine::F_Sprite> & t_chargingSprites, float t_chargeTime, float t_chargeRadius, int t_chargeMaxCount,float minSpeed, float maxSpeed)
{
    if(mp_targetPos)
    {
        ...
        float calculateXPos = (*mp_targetPos).x ;
        float calculateYPos = (*mp_targetPos).y ;
        ...
        for(int i = 0; i < m_chargeMaxCount; i++)
        {
            ...
            ChargeParticle * chargeParticle  = new ChargeParticle(glm::vec2(
                feint_common::Instance()->getRandomNum(calculateXPos - t_chargeRadius, calculateXPos + t_chargeRadius), feint_common::Instance()->getRandomNum(calculateYPos - t_chargeRadius, calculateYPos + t_chargeRadius)),
                t_chargingSprites[spriteIndex],
                feint_common::Instance()->getRandomNum(0,3.1415f),
                t_scale, t_scale * 0.05f , t_chargeTime);
            chargeParticle->setChargeMoveSpeed(feint_common::Instance()->getRandomNum(minSpeed, maxSpeed));
