/* 
	...
	now modified by : Feint ( 2016 May)
*/
...
#define LAST_PRINTABLE_CHAR 128
    ...
    class SpriteFont {
    public:
        SpriteFont(const char* font, int size, int cs, int ce);
		...
		glm::vec2 measure(const wchar_t* s);
		...
		void drawTextWithBlackBorder(SpriteBatch& batch, const char* s, glm::vec2 position, glm::vec2 scaling,
			float depth, Color tint, Justification just = Justification::LEFT);
    ...
    private:
        ...
        CharGlyph* _glyphs;
        int _fontHeight;
		glm::vec4 tempUV = glm::vec4(0, 0, 1.0f, 1.0f);
        unsigned int _texID;
    };
