...
namespace Feintgine
{
	...
	Feintgine::Mesh Model::processMesh(aiMesh *mesh, const aiScene *scene)
	{
		// data to fill
	...
	{
		std::vector<Texture3D> textures;
		//std::cout << "loadMaterialTextures called \n";
		for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
		{
			...
			{   // if texture hasn't been loaded already, load it
				...
				//std::cout << "load texture from path " << str.C_Str();
				Texture3D texture;
	...
	unsigned int Model::TextureFromFile(const char *path, const std::string &directory, bool gamma)
	{
		...
		//std::cout << "load data from path " << filename << "\n";
