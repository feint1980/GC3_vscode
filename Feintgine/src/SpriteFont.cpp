...
namespace Feintgine {
    ...
    SpriteFont::SpriteFont(const char* font, int size, int cs, int ce) {
        // Initialize SDL_ttf
		std::cout << "Initialize SDL_ttf from " << cs <<" to " << ce << " \n";
        if (!TTF_WasInit()) {
            TTF_Init();
        }
        ...
        std::cout << "pass init SDL_ttf part 1\n";
	...
	glm::vec2 SpriteFont::measure(const wchar_t* s) {
		glm::vec2 size(0, _fontHeight);
		float cw = 0;
		for (int si = 0; s[si] != 0; si++) {
			char c = s[si];
    ...
    void SpriteFont::draw(SpriteBatch& batch, const char* s, glm::vec2 position, glm::vec2 scaling, 
        ...
        for (int si = 0; s[si] != 0; si++) {
            ...
            if (s[si] == '\n') {
				...
				//std::cout << _glyphs[gi].uvRect.x << "glyp " << gi << std::endl;
				//batch.draw(destRect_b, _glyphs[gi].uvRect, _texID, depth - 1, Feintgine::Color(0, 0, 0, 255), 0);
				batch.draw(destRect, _glyphs[gi].uvRect, _texID, depth, tint, 0);
	...
	void SpriteFont::drawTextWithBlackBorder(SpriteBatch& batch, const char* s, glm::vec2 position, glm::vec2 scaling, float depth, Color tint, Justification just /*= Justification::LEFT*/)
	{
		glm::vec2 border = scaling + 0.1f;
		draw(batch, s, position, border, depth, Feintgine::Color(0, 0, 0, 255), just);
		//draw(batch, s, position * 0.99f, scaling, depth, tint, just);
