		...
		for(i = 0 ; i < rayCount ; i++)
     	{
     		...
     		if(fragmentPosition.y > rayLightPos[i].y - 650.0f) // 650 is the value offset of Y
     		{


	     		if(fragmentPosition.x > rayLightPos[i].x - (radius * 0.5f) && fragmentPosition.x < rayLightPos[i].x + (radius * 0.5f) /*&& fragmentPosition.y > rayLightPos[i].y - 1660.0f*/ )
	     		{
	     			float lapp = length(aux) * 100/ distance(fragmentPosition.y,rayLightPos[i].y - 650.0f);

	     			// if(fragmentPosition.y > rayLightPos[i].y - 650.0f)
	     			// {
	     			// 	lapp = length(aux * 1.0f/ distance(fragmentPosition,rayLightPos[i]) * 300.0f);
	     			// }
     		...
     		// else
     		// {
     		// 	float lapp = length(aux) * distance(fragmentPosition.y,rayLightPos[i].y - 650.0f) ;
