...
MessageHandling[PacketChannel.UserChannel][UserResponse.SkillInfo] = function(host ,data, ip, guid)
    ...
    if CH_FindClient(t_guid) ~= nil then
        ...
        SendReliable(host,ip,PacketChannel.UserChannel,UserResponse.SkillInfo_Start,{t_guid, "sequence_start"})
        for k,v in pairs(Skill_Serialized_Table) do
            for k2,v2 in pairs(v) do
                SendReliable(host,ip,PacketChannel.UserChannel,UserResponse.SkillInfo_Data,{k,k2,v2})
            end
        end
        SendReliable(host,ip,PacketChannel.UserChannel,UserResponse.SkillInfo_End,{"ev"})
    else
        print("invalid user")
        return
    end
