...
void main() {
		...
		int i;
		float t_length = 1300.0f;
		for(i = 0 ; i < rayCount ; i++)
     	{
     		float radius = 18000.0f / rayLightAtten[i].z;
     		...
     		//if(cos(angle) * (position.y - pos1.y) >= sin(angle) * (position.x - pos1.x) - 0.001 &&
	  		//cos(angle) * (position.y - pos1.y) <= sin(angle) * (position.x - pos1.x) + 0.001 )   
     			 ...
     			 // if((mod(rayRotation[i],6.28318530718f) >= 0.0f && 
				     ...
				     //   && fragmentPosition.y <= rayLightPos[i].y) )
	     		 // 	{
     				// if(fragmentPosition.y * sin(rayRotation[i]) > ( rayLightPos[i].y ) &&  )

     				// {


	     				//if(    sin(rayRotation[i]  + 1.57079632679f) * ( fragmentPosition.y - rayLightPos[i].y) + radius  >= cos(rayRotation[i]  + 1.57079632679f) * (fragmentPosition.x - rayLightPos[i].x) - radius
	 					//	&& sin(rayRotation[i]  + 1.57079632679f) * ( fragmentPosition.y - rayLightPos[i].y) - radius  <= cos(rayRotation[i]  + 1.57079632679f) * (fragmentPosition.x - rayLightPos[i].x) + radius)
	 		  	  // 650 is the value offset of Y
	 					

     					vec2 endPoint = vec2(rayLightPos[i].x + t_length * cos(rayRotation[i]), rayLightPos[i].y + t_length * sin(rayRotation[i]));
     					float f_length = sqrt( pow(fragmentPosition.x - rayLightPos[i].x,2.0) + pow(fragmentPosition.y - rayLightPos[i].y,2.0) );
			    ...
			    //  			finalColor.a = fragmentColor.a;
	 						float a = (fragmentPosition.x - rayLightPos[i].x);
							float b = (fragmentPosition.y - rayLightPos[i].y);
							float c = (endPoint.x - rayLightPos[i].x);
							float d = (endPoint.y - rayLightPos[i].y);
							if( (a * c  + b * d) <=  (sqrt( a * a  + b * b ) * sqrt( c * c + d * d ) ) +  radius  
							  &&  (a * c  + b * d) >=  (sqrt( a * a  + b * b ) * sqrt( c * c + d * d ) ) - radius  )
							{
