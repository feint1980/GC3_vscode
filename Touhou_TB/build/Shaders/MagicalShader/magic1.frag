...
void main(void)
{
	...
	float w = 1.04;
	gl_FragColor=vec4(vec3(gl_FragColor.rgb*w),1);
	
	const vec4 mul = vec4(1.-0.0);
	const vec4 add = -1./256.*vec4(1);
	const float deeps = sqrt(5.);
	const float rounds = 8.;
	for(float i = 0.; i <= -0.001+rounds; i++){
		float ph = 1.2345 + 3.14159*2.*i/rounds + cos(gl_FragCoord.x*33.-time) + sin(-gl_FragCoord.y*33.+time) - sin(-(gl_FragCoord.y+gl_FragCoord.x)*14.+time);
		gl_FragColor = max(gl_FragColor, texture2D(backbuffer, (deeps*vec2(cos(ph), sin(ph))+gl_FragCoord.xy)/resolution)*mul+add);
		gl_FragColor = mix(gl_FragColor, texture2D(backbuffer, (deeps*vec2(cos(ph), sin(ph))+gl_FragCoord.xy)/resolution)*mul+add, 0.2);
	}
	gl_FragColor.rgb /= w * 0.5f;
}